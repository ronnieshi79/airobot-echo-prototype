...
export interface ServiceSlice {
  id: string;
  type: 'focus' | 'timer' | 'calendar' | 'podcast' | 'alarm' | 'logbook';
  title: string;
  description: string;
...
export function getContextServiceSlices(params: {
  context: ContextType;
  now: Date;
...
}): { slices: ServiceSlice[]; primaryReason: string; primaryAction: string } {
  const {
    context,
    now,
    schedules,
  ...
  // 1. Check for Active / Running Dynamic Slices first!
  if (ringingAlarmId) {
  ...
  // 2. Contextual Presets based on Context
  switch (context) {
    case 'deep_work': {
      if (!isFocusRunning) {
        slices.push({
  ...
  // Determine recommendation banner text
